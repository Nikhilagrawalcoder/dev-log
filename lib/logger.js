...
class Logger {
  ...
  _formatMessage(message) {
    ...
    if (typeof message !== "string") {
      try {
        ...
        return String(message); // Fallback to a simple string conversion
      }
    }
    return message;
  }
  ...
  log(level, message) {
    ...
    const outputMessage = this.format
    ...
    console.log(colorFunc(outputMessage));

    // Prepare the log file message with labels
    const logFileMessage = `[${level.toUpperCase()}] ${formattedMessage} at ${timestamp}`;
  ...
  clearLogs() {
    if (fs.existsSync(this.logFile)) {
      ...
      console.warn(
        chalk.yellow(
          `Log file "${this.logFile}" does not exist, skipping clear.`
        )
      );
    }
  }
